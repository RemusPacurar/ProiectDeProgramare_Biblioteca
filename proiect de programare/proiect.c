...
{
    if (argc != 3) {
        printf("Usage: %s <first name> <last name>\n", argv[0]);
        return 1;
    }
    ...
    FILE *carti_file = fopen("C:\\Users\\rmspa\\proiect\\test2\\carti.txt", "r");
    if (carti_file == NULL) {
        printf("Error opening the file 'carti.txt'.\n");
        return 1;
    }

    FILE *imprumuturi_file = fopen("C:\\Users\\rmspa\\proiect\\test2\\carti.txt", "r");
    if (imprumuturi_file == NULL) {
        printf("Error opening the file 'imprumuturi.txt'.\n");
        return 1;
    }
...
void afiseaza_carti_imprumutate() {
    //system("cls"); // Curăță ecranul consolei
    printf("Cartile imprumutate de %s %s %s:\n", nume_utilizator, prenume_utilizator);
